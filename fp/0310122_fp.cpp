...
int indexToCcc(int i, int j) { return i * width + j + 1; }
...
void doPrint() {
    ...
    for (int i(1); i < width - 1; ++i) {
        for (int j(height - 1); j > 0; --j) {
            if (answer_table[j][i] == target_table[j][i] &&
                fixed_table[j][i - 1] ==
                    true)  // same color and the left one is true
            {
...
int main(int argc, char **argv) {
    ...
    for (int i(0); static_cast<unsigned int>(i) < answer_lists.size(); ++i) {
        for (int j(0); static_cast<unsigned int>(j) < answer_lists.at(i).size();
             ++j) {
            // outFile << setw(3);
